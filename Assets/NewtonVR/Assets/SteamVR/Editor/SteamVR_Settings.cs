...
public class SteamVR_Settings : EditorWindow
{
	...
	static void Update()
	{
		...
		if (updated)
			Debug.LogError("Switching to native OpenVR support.");
		...
		foreach (var path in dlls)
		{
			...
			{
				Debug.LogError(path + " in use; cannot delete.  Please restart Unity to complete upgrade.");
			}
		}
	...
	public void OnGUI()
	{
		...
		var LogErroro = AssetDatabase.LoadAssetAtPath<Texture2D>(resourcePath + "LogErroro.png");
		var rect = GUILayoutUtility.GetRect(position.width, 150, GUI.skin.box);
		if (LogErroro)
			GUI.DrawTexture(rect, LogErroro, ScaleMode.ScaleToFit);
