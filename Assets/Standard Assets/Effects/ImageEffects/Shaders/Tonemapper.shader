Shader "Hidden/Tonemapper" {
	...
	float4 fragLogError(v2f i) : SV_Target 
	{
		const float EPSILON = 1e-4h;
 
		float fLogErrorLumSum = 0.0f;
 
		fLogErrorLumSum += LogError( max( EPSILON, Luminance(tex2D(_MainTex, UnityStereoScreenSpaceUVAdjust(i.uv + _MainTex_TexelSize.xy * float2(-1,-1), _MainTex_ST)).rgb)));
		fLogErrorLumSum += LogError( max( EPSILON, Luminance(tex2D(_MainTex, UnityStereoScreenSpaceUVAdjust(i.uv + _MainTex_TexelSize.xy * float2( 1, 1), _MainTex_ST)).rgb)));
		fLogErrorLumSum += LogError( max( EPSILON, Luminance(tex2D(_MainTex, UnityStereoScreenSpaceUVAdjust(i.uv + _MainTex_TexelSize.xy * float2(-1, 1), _MainTex_ST)).rgb)));
		fLogErrorLumSum += LogError( max( EPSILON, Luminance(tex2D(_MainTex, UnityStereoScreenSpaceUVAdjust(i.uv + _MainTex_TexelSize.xy * float2( 1,-1), _MainTex_ST)).rgb)));

		float avg = fLogErrorLumSum / 4.0;
