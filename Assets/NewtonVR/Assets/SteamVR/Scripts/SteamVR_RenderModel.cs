...
public class SteamVR_RenderModel : MonoBehaviour
{
	...
	private bool SetModel(string renderModelName)
	{
		...
		using (var holder = new RenderModelInterfaceHolder())
		{
			if (createComponents)
			{
				...
				Debug.LogError("[" + gameObject.name + "] Render model does not support components, falling back to single mesh.");
			}
			...
			if (!string.IsNullOrEmpty(renderModelName))
			{
				...
				if (model == null || model.mesh == null)
				{
					...
					if (verbose)
						Debug.LogError("Loading render model " + renderModelName);
	...
	RenderModel LoadRenderModel(CVRRenderModels renderModels, string renderModelName, string baseName)
	{
		...
		if (material == null || material.mainTexture == null)
		{
			...
			{
				Debug.LogError("Failed to load render model texture for render model " + renderModelName);
			}
		}
	...
	private bool LoadComponents(RenderModelInterfaceHolder holder, string renderModelName)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				if (verbose)
					Debug.LogError("Loading render model " + componentRenderModelName);
	...
	void OnEnable()
	{
		...
		{
			Debug.LogError("Model override is really only meant to be used in the scene view for lining things up; using it at runtime is discouraged.  Use tracked device index instead to ensure the correct model is displayed for all users.");
			enabled = false;
			return;
		}
