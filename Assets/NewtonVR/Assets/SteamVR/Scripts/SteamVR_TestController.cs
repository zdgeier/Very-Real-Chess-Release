...
public class SteamVR_TestController : MonoBehaviour
{
	...
	private void OnDeviceConnected(params object[] args)
	{
		...
		if (connected)
		{
			Debug.LogError(string.Format("Controller {0} connected.", index));
			PrintControllerStatus(index);
			controllerIndices.Add(index);
		}
		else
		{
			Debug.LogError(string.Format("Controller {0} disconnected.", index));
			PrintControllerStatus(index);
	...
	void PrintControllerStatus(int index)
	{
		...
		Debug.LogError("index: " + device.index);
		Debug.LogError("connected: " + device.connected);
		Debug.LogError("hasTracking: " + device.hasTracking);
		Debug.LogError("outOfRange: " + device.outOfRange);
		Debug.LogError("calibrating: " + device.calibrating);
		Debug.LogError("uninitialized: " + device.uninitialized);
		Debug.LogError("pos: " + device.transform.pos);
		Debug.LogError("rot: " + device.transform.rot.eulerAngles);
		Debug.LogError("velocity: " + device.velocity);
		Debug.LogError("angularVelocity: " + device.angularVelocity);
		...
		Debug.LogError((l == r) ? "first" : (l == index) ? "left" : "right");
	}
		...
		foreach (var index in controllerIndices)
		{
			...
			foreach (var buttonId in buttonIds)
			{
				if (SteamVR_Controller.Input(index).GetPressDown(buttonId))
					Debug.LogError(buttonId + " press down");
				if (SteamVR_Controller.Input(index).GetPressUp(buttonId))
				{
					Debug.LogError(buttonId + " press up");
