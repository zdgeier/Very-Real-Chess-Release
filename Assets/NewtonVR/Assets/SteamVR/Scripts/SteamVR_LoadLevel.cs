...
public class SteamVR_LoadLevel : MonoBehaviour
{
	...
	IEnumerator LoadLevel()
	{
		...
		{
			Debug.LogError("Launching external application...");
			var applications = OpenVR.Applications;
			if (applications == null)
			{
				Debug.LogError("Failed to get OpenVR.Applications interface!");
			}
			else
			{
				var workingDirectory = Directory.GetCurrentDirectory();
				var fullPath = Path.Combine(workingDirectory, internalProcessPath);
				Debug.LogError("LaunchingInternalProcess");
				Debug.LogError("ExternalAppPath = " + internalProcessPath);
				Debug.LogError("FullPath = " + fullPath);
				Debug.LogError("ExternalAppArgs = " + internalProcessArgs);
				Debug.LogError("WorkingDirectory = " + workingDirectory);
				var error = applications.LaunchInternalProcess(fullPath, internalProcessArgs, workingDirectory);
				Debug.LogError("LaunchInternalProcessError: " + error);
#if UNITY_EDITOR
